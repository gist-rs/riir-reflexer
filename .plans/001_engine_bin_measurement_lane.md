...
**Status:** OPEN — filed 2026-09-25 after P0 (katgpt-tetris `243c38a1b`) and
...
- [ ] T1 Workspace + skeleton: root package `reflexer` (bin `reflexer`),
...
- [ ] T2 Engine core: load the REFERENCE genome
      ...
      re-derive).
- [ ] T3 Serving edge: line-JSON over stdin/stdout; one request → one
      response envelope; malformed input → typed error envelope, never a
      panic; EOF exits 0.
- [ ] T4 The envelope carries `in_engine_decision_ns` (the engine's own
      in-process decision time) so harnesses render it beside the
      round-trip — never display a sub-ms decision through a same-size
      subprocess overhead.
- [ ] T5 Measurement lane (bin-only, the arena posture): subprocess the
...
- [ ] T6 Trajectory submission client (free, opt-in, OFF by default):
...
- [ ] T7 G1 gate: champion replay bit-identity — the bin's per-seed
...
- [ ] T8 G2 gate: matched-time latency budget under `--release` with box
...
- [ ] T9 Docs: README (build + run + the measurement contract), AGENTS
